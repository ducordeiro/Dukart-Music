...
const mediaInfoCache = new Map<string, { expiresAt: number; promise: Promise<MediaInfo> }>();
...
export function readMediaInfo(url: string): Promise<MediaInfo> {
  const cacheKey = youtubeVideoId(url) || url.trim();
  const cached = mediaInfoCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.promise;
  }

  const promise = readMediaInfoUncached(url);
  mediaInfoCache.set(cacheKey, { expiresAt: Date.now() + MEDIA_INFO_CACHE_TTL_MS, promise });
  void promise.catch(() => {
    if (mediaInfoCache.get(cacheKey)?.promise === promise) {
      mediaInfoCache.delete(cacheKey);
    }
  });

  if (mediaInfoCache.size > 100) {
    const now = Date.now();
    for (const [key, entry] of mediaInfoCache) {
      if (entry.expiresAt <= now) mediaInfoCache.delete(key);
    }
  }
...
async function readMediaInfoUncached(url: string): Promise<MediaInfo> {
...
export async function downloadMedia(
  ...
  const sharedArgs = [...youtubeExtractorArgs(), "--no-playlist", "--newline", "--progress", ...ffmpegLocation];
  const args =
    type === "audio"
      ? [
          ...sharedArgs,
...
export function downloadErrorMessage(error: unknown) {
  ...
  if (normalized.includes("http error 403") || normalized.includes("po token")) {
    return "O YouTube recusou este download. Tente novamente; se o erro persistir, atualize o suporte do YouTube no servidor.";
  }
...
export function youtubeDownloadSupportStatus() {
  const pluginDirectory = configuredDirectory("ESPORTE_FAI_YTDLP_PLUGIN_DIR");
  const providerHome = configuredDirectory("ESPORTE_FAI_YTDLP_PROVIDER_HOME");
  const playerClient = configuredPlayerClient();
  return {
    configured: Boolean(pluginDirectory && providerHome && playerClient),
...
function youtubeExtractorArgs() {
  ...
  if (providerHome) {
    args.push("--extractor-args", `youtubepot-bgutilscript:server_home=${providerHome}`);
  }

  return args;
}

function configuredDirectory(environmentKey: string) {
...
function configuredPlayerClient() {
  const configured = process.env.ESPORTE_FAI_YOUTUBE_PLAYER_CLIENT?.trim().toLocaleLowerCase("en-US") || "";
  return /^[a-z0-9_,-]+$/.test(configured) ? configured : "";
}
